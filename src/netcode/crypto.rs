...
use chacha20poly1305::{
    AeadInPlace, ChaCha20Poly1305, Error as CryptoError, Key, KeyInit, Nonce, Tag,
...
pub fn dencrypted_in_place(
    ...
    let (buffer, tag) = buffer.split_at_mut(buffer.len() - MAC_BYTES);
...
pub fn dencrypted_in_place_xnonce(
    buffer: &mut [u8],
    xnonce: &[u8; 24],
    private_key: &[u8; 32],
    aad: &[u8],
...
pub fn encrypt_in_place(
    ...
    nonce[4..12].copy_from_slice(&sequence.to_le_bytes());
    let nonce = Nonce::from(nonce);
    let (buffer, buffer_tag) = buffer.split_at_mut(buffer.len() - MAC_BYTES);
...
pub fn encrypt_in_place_xnonce(
    buffer: &mut [u8],
    xnonce: &[u8; 24],
    key: &[u8; 32],
    aad: &[u8],
...
mod tests {
    ...
    fn test_encrypt_decrypt_in_place() {
        ...
        data.extend_from_slice(&[0u8; MAC_BYTES]);

...
impl SafePassword {
    ...
    pub fn parse(s: String) -> Result<SafePassword, String> {
        ...
        if is_empty_or_whitespace || is_too_long || is_too_short || contains_forbidden_characters {
            Err(format!("{s} is not a valid password"))
        } else {
            Ok(Self(s))
        }
    }
...
impl AsRef<str> for SafePassword {
    fn as_ref(&self) -> &str {
        &self.0
    }
}

...
impl<'a> Event<'a> {
  ...
  #[inline]
  #[allow(non_snake_case)]
  pub fn data_as_server_whisper_receipt(&self) -> Option<ServerWhisperReceipt<'a>> {
    if self.data_type() == EventData::ServerWhisperReceipt {
      let u = self.data();
      // Safety:
      // Created from a valid Table for this object
      // Which contains a valid union in this slot
      Some(unsafe { ServerWhisperReceipt::init_from_table(u) })
    } else {
      None
    }
  }
    ...
    v.visit_table(pos)?
     .visit_union::<EventData, _>("data_type", Self::VT_DATA_TYPE, "data", Self::VT_DATA, true, |key, v, pos| {
        match key {
          ...
          EventData::ServerWhisperReceipt => v.verify_union_variant::<flatbuffers::ForwardsUOffset<ServerWhisperReceipt>>("EventData::ServerWhisperReceipt", pos),
    ...
    let mut ds = f.debug_struct("Event");
      ...
      match self.data_type() {
        ...
        EventData::ServerWhisperReceipt => {
          if let Some(x) = self.data_as_server_whisper_receipt() {
            ds.field("data", &x)
          } else {
            ds.field("data", &"InvalidFlatbuffer: Union discriminant does not match value.")
          }

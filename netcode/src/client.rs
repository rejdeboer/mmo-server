...
use crate::channel_packet::{ChannelPacket, Payload};
...
/// The connection status of a [`NetcodeClient`].
#[derive(Debug)]
...
impl NetcodeClient {
    ...
    fn from_channels(
        ...
        let mut channel_send_order: Vec<ChannelOrder> =
            Vec::with_capacity(send_channels_config.len());
        ...
        for channel_config in receive_channels_config.iter() {
            match channel_config.send_type {
                SendType::Unreliable => {
                    ...
                    let old =
                        receive_unreliable_channels.insert(channel_config.channel_id, channel);
                    assert!(
                        old.is_none(),
                ...
                SendType::ReliableOrdered { .. } => {
                    let channel =
                        ReceiveChannelReliable::new(channel_config.max_memory_usage_bytes, true);
                ...
                SendType::ReliableUnordered { .. } => {
                    let channel =
                        ReceiveChannelReliable::new(channel_config.max_memory_usage_bytes, false);
                    let old = receive_reliable_channels.insert(channel_config.channel_id, channel);
                    assert!(
                        old.is_none(),
                        "already exists receive channel {}",
    ...
    pub fn is_disconnected(&self) -> bool {
        matches!(
            ...
            RenetConnectionStatus::Disconnected { .. }
        )
    }
    ...
    pub fn receive_message<I: Into<u8>>(&mut self, channel_id: I) -> Option<Bytes> {
        ...
        } else if let Some(unreliable_channel) =
            self.receive_unreliable_channels.get_mut(&channel_id)
        {
    ...
    pub fn process_packet(&mut self, packet: &[u8]) {
        ...
        let packet = match ChannelPacket::from_bytes(&mut octets) {
            Err(err) => {
        ...
        match packet {
            ChannelPacket::SmallReliable {
                channel_id,
                messages,
                ..
            } => {
                let Some(channel) = self.receive_reliable_channels.get_mut(&channel_id) else {
                    self.disconnect_with_reason(DisconnectReason::ReceivedInvalidChannelId(
                        channel_id,
                    ));
                    return;
                };
                    ...
                    match sent_packet.info {
                        PacketSentInfo::ReliableMessages {
                            ...
                            let reliable_channel =
                                self.send_reliable_channels.get_mut(&channel_id).unwrap();
    ...
    pub fn get_packets_to_send(&mut self) -> Vec<Payload> {
        let mut packets: Vec<ChannelPacket> = vec![];
        if self.is_disconnected() {
            return vec![];
        }
        ...
        for order in self.channel_send_order.iter() {
            match order {
                ...
                ChannelOrder::Unreliable(channel_id) => {
                    ...
                    packets.append(
                        &mut channel
                            .get_packets_to_send(&mut self.packet_sequence, &mut available_bytes),
                    );
                }
            }
        }
        ...
        if !self.pending_acks.is_empty() {
            let ack_packet = ChannelPacket::Ack {
        ...
        for packet in packets.iter() {
            match packet {
                ...
                ChannelPacket::SmallUnreliable { sequence, .. } => {
                    self.sent_packets.insert(
                        *sequence,
                        PacketSent {
                            sent_at,
                ...
                ChannelPacket::UnreliableSlice { sequence, .. } => {
                    self.sent_packets.insert(
                        *sequence,
                        PacketSent {
                            sent_at,
    ...
    fn add_pending_ack(&mut self, sequence: u64) {
        ...
        for index in 0..self.pending_acks.len() {
            ...
            if range.start == sequence + 1 {
                ...
                if next_index < self.pending_acks.len()
                    && self.pending_acks[index].end == self.pending_acks[next_index].start
                {
...
mod tests {
    ...
    #[test]
    fn pending_acks() {
        let mut connection = NetcodeClient::new(ConnectionConfig::default());

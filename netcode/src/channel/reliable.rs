...
use crate::{
    channel_packet::{ChannelPacket, SLICE_SIZE, Slice},
    error::ChannelError,
};
...
impl SendChannelReliable {
                ...
                UnackedMessage::Small { message, last_sent } => {
                    ...
                    *available_bytes -= message.len() as u64;
                    ...
                    if small_messages_bytes + serialized_size > SLICE_SIZE {
                        packets.push(ChannelPacket::SmallReliable {
                ...
                UnackedMessage::Sliced {
                    ...
                    for i in 0..*num_slices {
                        ...
                        *available_bytes -= payload.len() as u64;
                        ...
                        packets.push(ChannelPacket::ReliableSlice {
                            sequence: *packet_sequence,
                            channel_id: self.channel_id,
                            slice,
                        });
    ...
    pub fn send_message(&mut self, message: Bytes) -> Result<(), ChannelError> {
        ...
        self.unacked_messages
            .insert(self.next_reliable_message_id, unacked_message);
...
impl ReceiveChannelReliable {
    ...
    pub fn process_slice(&mut self, slice: Slice) -> Result<(), ChannelError> {
        if self.messages.contains_key(&slice.message_id)
            || slice.message_id < self.oldest_pending_message_id
        {
...
mod tests {
    ...
    fn small_packet() {
        ...
        for packet in packets {
            let ChannelPacket::SmallReliable {
                sequence: 0,
                channel_id: 0,
                messages,
            } = packet
    ...
    fn small_packet_unordered() {
        ...
        let packets = send.get_packets_to_send(&mut sequence, &mut available_bytes, current_time);
        assert_eq!(packets.len(), 1);
        let ChannelPacket::SmallReliable { messages, .. } = &packets[0] else {
            unreachable!();
        };
        ...
        recv.process_message(messages[2].1.clone(), messages[2].0)
            .unwrap();
    ...
    fn slice_packet() {
        ...
        let packets = send.get_packets_to_send(&mut sequence, &mut available_bytes, current_time);
        for packet in packets {
            let ChannelPacket::ReliableSlice {
                channel_id: 0,
                slice,
                ..
            } = packet

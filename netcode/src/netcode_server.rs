...
use crate::ClientId;
use crate::error::NetcodeTransportError;
use crate::server::RenetServer;
use crate::{
    CONNECT_TOKEN_PRIVATE_BYTES, CONNECT_TOKEN_XNONCE_BYTES, KEY_BYTES, MAC_BYTES, MAX_CLIENTS,
    MAX_PACKET_BYTES, MAX_PAYLOAD_BYTES, MAX_PENDING_CLIENTS, SEND_RATE, USER_DATA_BYTES,
    VERSION_INFO,
...
pub struct NetcodeServer {
    ...
    connect_token_entries: Box<[Option<ConnectTokenEntry>; MAX_CLIENTS * 2]>,
    protocol_id: u64,
    connect_key: [u8; KEY_BYTES],
    max_clients: usize,
...
pub enum ServerResult<'a, 's> {
    ...
    ClientConnected {
        client_id: u64,
        addr: SocketAddr,
        user_data: Box<[u8; USER_DATA_BYTES]>,
        payload: &'s mut [u8],
    },
...
impl NetcodeServer {
    pub fn new(config: ServerConfig) -> Self {
        if config.max_clients > MAX_CLIENTS {
            ...
            panic!("The max clients allowed is {}", MAX_CLIENTS);
        }
        ...
        let connect_key = match config.authentication {
            ServerAuthentication::Unsecure => [0; KEY_BYTES],
        ...
        Self {
            clients,
            connect_token_entries: Box::new([None; MAX_CLIENTS * 2]),
    ...
    pub fn user_data(&self, client_id: u64) -> Option<[u8; USER_DATA_BYTES]> {
    ...
    fn handle_connection_request<'a>(
        ...
        let connect_token = PrivateConnectToken::decode(
            &data,
            self.protocol_id,
            expire_timestamp,
            &xnonce,
        ...
        let id_already_connected =
            find_client_mut_by_id(&mut self.clients, connect_token.client_id).is_some();
        ...
        if !self.pending_clients.contains_key(&addr)
            && self.pending_clients.len() >= MAX_PENDING_CLIENTS
        {
            tracing::warn!(
        ...
        mac.copy_from_slice(&data[CONNECT_TOKEN_PRIVATE_BYTES - MAC_BYTES..]);
    ...
    fn process_packet_internal<'a, 's>(
        ...
        if let Some(pending) = self.pending_clients.get_mut(&addr) {
            ...
            match packet {
                ...
                Packet::Response {
                    ...
                    let challenge_token =
                        ChallengeToken::decode(token_data, token_sequence, &self.challenge_key)?;
                    ...
                    match self.clients.iter().position(|c| c.is_none()) {
                        ...
                        Some(client_index) => {
                            ...
                            let user_data: [u8; USER_DATA_BYTES] = pending.user_data;
        ...
        match packet {
            ...
            _ => unreachable!(
                "Decoding packet without key can only return ConnectionRequest packets"
            ),
        }
    }
    ...
    pub fn set_max_clients(&mut self, max_clients: usize) {
        let max_clients = max_clients.min(MAX_CLIENTS);
        tracing::debug!("Netcode max_clients set to {}", max_clients);
    ...
    /// # Example
    /// ```
    /// # use netcode::netcode_server::ServerResult;
    /// # let mut server = netcode::netcode_server::NetcodeServer::__test();
    ...
    pub fn update_client(&mut self, client_id: u64) -> ServerResult<'_, '_> {
        ...
        if let Some(client) = &mut self.clients[slot] {
            ...
            if client.last_packet_send_time + SEND_RATE <= self.current_time {
                let packet = Packet::KeepAlive {
...
impl NetcodeServerTransport {
    ...
    pub fn user_data(&self, client_id: ClientId) -> Option<[u8; USER_DATA_BYTES]> {
...
mod tests {
    use crate::{
        netcode_client::{ClientAuthentication, PlaceholderClient},
        token::ConnectToken,
    };

    use super::*;

    const TEST_KEY: &[u8; KEY_BYTES] = b"an example very very secret key."; // 32-bytes
    ...
    fn server_connection() {
        ...
        let mut client = PlaceholderClient::new(Duration::ZERO, client_auth).unwrap();

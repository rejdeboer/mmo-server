...
use crate::{
    ...
    components::{ClientIdComponent, GridCell, VisibleEntities},
...
pub fn update_player_visibility(
        ...
        q_players.iter_mut()
    {
            ...
            for x in -1..=1 {
                ...
                if let Some(cell_entities) = grid.cells.get(&cell_coords) {
                    for &other_entity in cell_entities {
                        ...
                        if let Ok(other_transform) = q_transforms.get(other_entity) {
        ...
        for &entity_to_spawn in new_visible_set.difference(&visible.entities) {
            if let Ok(transform) = q_transforms.get(entity_to_spawn) {
